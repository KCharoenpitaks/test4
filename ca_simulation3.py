...
def get_recent_date():
    ...
    cur = conn.cursor()
    cur.execute("""SELECT DISTINCT asof
...
def get_trade_sum(date):
    ...
    if os.name == 'nt':
        ...
        api_url = r'http://192.168.50.41:8444/api/v1/dailyreport/tradetrx/view/%(date)s' % {'date': today_str}
...
def simulation_1day(date):
    ...
    #prev_date = '2021-01-06' #"'2018-12-27'" #"'2019-03-27'"
    prev_date = str(get_recent_date().values[0][0])
    ...
    df_update = pd.merge(df_starting_data,df,how='outer', left_on = 'symbol', right_on = 'symbol')
